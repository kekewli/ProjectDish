...
namespace ProjectDish.MVVM.ViewModels
{
    internal class PasswordResetViewModel : ViewModelBase
    {
        ...
        private async Task ExecuteSendCode()
        {
            ...
            try
            {
                // Запрос в БД всегда, чтобы скрыть существование email
                var rpcParams = new { p_email = Email.ToLowerInvariant() };
                DataTable dt = await DatabaseHelper.ExecuteQuery("create_password_reset_token", rpcParams);
                ...
                // Информируем пользователя в любом случае
                MessageBox.Show("Если email зарегистрирован в системе, на него будет отправлен код восстановления.", "Информация", MessageBoxButton.OK, MessageBoxImage.Information);
                CurrentStep = 2;
            }
            catch (Exception ex)
            {
        ...
        private async Task ExecuteResetPassword(object parameter)
        {
            ...
            {
                MessageBox.Show("Пароль должен быть от 8 до 64 символов и содержать буквы и цифры.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error); return;
            }
            ...
            try
            {
                ...
                if (ok)
                {
                    ...
                    MessageBox.Show("Пароль успешно изменен. Теперь вы можете войти.", "Готово", MessageBoxButton.OK, MessageBoxImage.Information);
                ...
                else
                {
                    ...
                    MessageBox.Show("Код неверный или устарел.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
